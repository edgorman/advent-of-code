                ...
                if v[1] == v_last[1]:
                    ...
                    if u[1] <= v[1] <= u_last[1] or u_last[1] <= v[1] <= u[1]:
                        ...
                        if v[0] <= u[0] <= v_last[0] or v_last[0] <= u[0] <= v[0]:
                            # Crossing at ux, vy
                            crossing_points.append((u[0], v[1]))
                ...
                if v[0] == v_last[0]:
                    ...
                    if u[0] <= v[0] <= u_last[0] or u_last[0] <= v[0] <= u[0]:
                        ...
                        if v[1] <= u[1] <= v_last[1] or v_last[1] <= u[1] <= v[1]:
                            # Crossing at vx, uy
                            crossing_points.append((v[0], u[1]))
            
            v_last = v
        u_last = u

    # Find shortest crossing point from origin using manhatten distance
    crossing_distances = sorted([abs(x) + abs(y) for x, y in crossing_points])

    if crossing_distances[0] == 0:
        return crossing_distances[1]
    else:
